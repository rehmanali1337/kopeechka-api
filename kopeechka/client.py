...
from .errors import KopeechkaError
...
class KopeechkaClient:
    ...
    async def get_mail(
        ...
        if resp["status"] == "ERROR":
            print(f"error getting mail ... {resp}")
    ...
    async def get_verification_code(
        self, mail_id: str, full: Literal["0", "1"] = "0", check_every: int = 3
    ) -> str:
        ...
        if resp["status"] == "ERROR" and resp["value"] == "WAIT_LINK":
            print(
                f"Waiting for {check_every} seconds before checking for email message again ..."
            )
            await asyncio.sleep(check_every)

            return await self.get_verification_code(
                mail_id, full, check_every=check_every
            )
